...
void button_Short_Press()
{
  ...
  if (LCD_State == SCR_MENU)
    ...
    LCD_Display_program_delete(0, 1);
  else if (LCD_State == SCR_PROGRAM_FULL)
    LCD_Display_program_summary(0, 1);
  else if (LCD_State == SCR_QUICK_PROGRAM)
    LCD_Display_quick_program(0, 2);
  else
    LCD_display_menu(); // if pressed something else - go back to menu
}
...
void Rotate()
{
  ...
  else if (LCD_State == SCR_PROGRAM_LIST)
  {
    ...
    LCD_Display_quick_program(encoderValue, 1);
}
